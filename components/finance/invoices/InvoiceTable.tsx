...
export function InvoiceTable({
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    if (searchQuery) {
      ...
      if (!inv.invoiceNumber.toLowerCase().includes(searchLower) && !inv.clientName.toLowerCase().includes(searchLower)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-gray-200 flex flex-wrap gap-3">
        ...
        {hasActiveFilters && (
          <button onClick={onResetFilters} className="flex items-center gap-1 px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm">
            <RefreshCw size={14} /> Clear
          </button>
        )}
        <button onClick={onRefresh} disabled={loading} className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm ml-auto">
        ...
        <div className="px-5 py-2 bg-gray-50 border-b border-gray-200 flex flex-wrap gap-2">
          ...
          {statusFilter !== "ALL" && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-gray-200 rounded-full text-xs">
              Status: {statusFilter}
              <button onClick={() => onStatusFilterChange("ALL")} className="hover:text-red-500"><X size={12} /></button>
            </span>
          )}
          {searchQuery && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-gray-200 rounded-full text-xs">
              Search: "{searchQuery}"
              <button onClick={() => onSearchChange("")} className="hover:text-red-500"><X size={12} /></button>
            </span>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <EmptyState hasFilters={hasActiveFilters} onReset={onResetFilters} onCreateClick={onCreateClick} />
      ) : (
        <>
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${finalConfig.bg} ${finalConfig.text}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {inv.status !== "PAID" && inv.status !== "OVERDUE" && (
                            <select 
                              onChange={async (e) => { 
                                const ns = e.target.value as "SENT" | "PAID"; 
                            ...
                            >
                              ...
                              {inv.status === "SENT" && <option value="PAID">Mark Paid</option>}
                            </select>
                          )}
                        </div>
                      </td>
          ...
          {totalPages > 1 && (
            <div className="px-5 py-3 border-t border-gray-200 flex justify-between items-center">
              <span className="text-sm text-gray-500">
                Showing {((currentPage-1)*PAGE_SIZE)+1} to {Math.min(currentPage*PAGE_SIZE, filteredInvoices.length)} of {filteredInvoices.length}
              </span>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-8 h-8 text-sm rounded-lg ${currentPage===p ? "bg-blue-600 text-white" : "border border-gray-200 hover:bg-gray-50"}`}
                    >
                      {p}
                    </button>
                  );
