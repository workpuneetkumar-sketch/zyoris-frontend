...
export default function ViewLeadModal({ lead, onClose }: ViewLeadModalProps) {
  ...
  useEffect(() => {
    ...
    async function loadScore() {
      ...
      try {
        ...
        console.log("[ViewLeadModal] Got lead score:", result);
        if (mounted) {
          setScore(result.score);
        }
      } catch (error: any) {
        console.error("[ViewLeadModal] Failed to fetch lead score:", error);
        if (mounted) {
  ...
  const valuePts =
    val > 0
      ? Math.min(20, Math.round((Math.log10(val + 1) / Math.log10(100001)) * 20))
      : 0;
  const sourcePts = SOURCE_SCORE[lead.source ?? ""] ?? 8;
  const completePts = Math.min(
    8,
    (lead.name ? 2 : 0) +
      (lead.email ? 2 : 0) +
      (lead.phone ? 1 : 0) +
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                <DollarSign size={12} className="text-emerald-500" /> Estimated
                Value
              </p>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div key={d.label}>
                          ...
                          <div className="w-full h-1 bg-gray-200 rounded-full overflow-hidden">
                            <div
                              className="h-full rounded-full"
                              style={{
                                width: `${Math.round((d.pts / d.max) * 100)}%`,
                                background: d.color,
                              }}
                            />
                          </div>
              ...
              {scoreError && (
                <div className="mt-2 text-[10px] text-red-500">{scoreError}</div>
              )}
            </div>
          </div>
          ...
          {!hasContactInfo && !hasTags && !hasNote && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center">
              <p className="text-gray-400 text-sm">
                No additional information available for this lead.
              </p>
              <p className="text-gray-300 text-xs mt-1">
                Contact details and notes will appear here once added.
              </p>
            </div>
          )}
