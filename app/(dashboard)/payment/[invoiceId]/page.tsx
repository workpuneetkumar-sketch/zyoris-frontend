...
function PaymentPage() {
  ...
  const handleRazorpayPayment = useCallback(async () => {
    ...
    setProcessing(true);
    try {
      // Step 1: Create order (use retry if previously failed)
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-6 max-w-5xl mx-auto">
      ...
      {!isFullyPaid ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 mb-6 shadow-sm">
          ...
          {/* ── Razorpay Pay Button (Primary) ─────────────────── */}
          <button
            onClick={handleRazorpayPayment}
            disabled={processing}
            className="w-full inline-flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-bold rounded-xl shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.99] mb-2"
          >
            {processing ? (
              <>
